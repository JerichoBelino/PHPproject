...
if (isset($_POST['update'])) {
    try {
        ...
        $loanQuery = $user->getConnection()->prepare("SELECT * FROM `loan` WHERE `loan_id` = :loan_id");
        $loanQuery->bindParam(':loan_id', $loan_id, PDO::PARAM_INT);
        $loanQuery->execute();
        $fetch = $loanQuery->fetch(PDO::FETCH_ASSOC);

        // Fetch loan plan details securely using the $user object
        $lplanQuery = $user->getConnection()->prepare("SELECT * FROM `loan_plan` WHERE `lplan_id` = :lplan_id");
        ...
        if (preg_match('/[1-9]/', $fetch['date_released'])) {
            ...
            if ($fetch['status'] == 2) { // Status 2 indicates release
                ...
                for ($i = 1; $i <= $month; $i++) {
                    ...
                    $scheduleQuery = $user->getConnection()->prepare("INSERT INTO `loan_schedule` (`loan_id`, `due_date`) VALUES (:loan_id, :due_date)");
        ...
        $updateQuery = $user->getConnection()->prepare(
            "UPDATE `loan` 
